...
const About = () => {
  ...
  return (
    <div className="min-h-screen bg-[#D5E7B5] dark:bg-[#1a1a2e]">
      ...
      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-[#AE2448] to-[#6E1A37] py-16 md:py-24">
      ...
      <section className="container-custom py-12 md:py-20 px-4">
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-center">
          ...
          <div className="lg:w-1/2">
            ...
            <p className="text-[#2D3A2B]/70 dark:text-gray-300 leading-relaxed mb-4">
            ...
            <p className="text-[#2D3A2B]/70 dark:text-gray-300 leading-relaxed mb-6">
            ...
            <Link to="/shop" className="inline-flex items-center gap-2 bg-[#AE2448] text-white px-6 py-3 rounded-full font-semibold hover:bg-[#6E1A37] transition-all hover:scale-105">
      ...
      {/* Stats Section */}
      <section className="bg-white dark:bg-[#2a2a2a] py-12 md:py-16">
        <div className="container-custom px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {stats.map((stat, index) => (
              <div key={index} className="text-center">
                <div className="text-[#AE2448] mb-3 flex justify-center">{stat.icon}</div>
                <div className="text-2xl md:text-3xl font-bold text-[#2D3A2B] dark:text-white">{stat.value}</div>
                <div className="text-sm text-[#2D3A2B]/60 dark:text-gray-400">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="container-custom py-12 md:py-20 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {values.map((value, index) => (
            <div key={index} className="bg-white dark:bg-[#2a2a2a] p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all hover:-translate-y-2">
              <div className="w-12 h-12 rounded-full bg-[#AE2448]/10 flex items-center justify-center mb-4 text-[#AE2448] text-xl">
                {value.icon}
              </div>
              <h3 className="text-lg font-bold text-[#2D3A2B] dark:text-white mb-2">{value.title}</h3>
              <p className="text-sm text-[#2D3A2B]/60 dark:text-gray-400">{value.desc}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {team.map((member, index) => (
              <div key={index} className="text-center group">
                <div className="w-32 h-32 mx-auto rounded-full overflow-hidden mb-4 ring-4 ring-[#AE2448]/20 group-hover:ring-[#AE2448] transition-all">
                  <img src={member.image} alt={member.name} className="w-full h-full object-cover" />
                </div>
                <h3 className="font-bold text-[#2D3A2B] dark:text-white">{member.name}</h3>
                <p className="text-sm text-[#2D3A2B]/60 dark:text-gray-400">{member.role}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA Section */}
      <section className="container-custom py-12 md:py-20 px-4">
        <div className="bg-gradient-to-r from-[#AE2448] to-[#6E1A37] rounded-3xl p-10 md:p-16 text-center text-white">
          <h3 className="text-2xl md:text-3xl font-bold mb-3">Ready to start shopping?</h3>
          <p className="text-white/80 mb-6 max-w-md mx-auto">Join thousands of happy customers who trust NexCart</p>
          <Link to="/shop" className="inline-flex items-center gap-2 bg-white text-[#AE2448] px-8 py-3 rounded-full font-semibold hover:shadow-xl transition-all hover:scale-105">
